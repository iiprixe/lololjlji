...
do -- Example UI
	local Window = library:AddWindow("Galaxa Hub | Evade", {
		main_color = Color3.fromRGB(143, 67, 255),
	...
	do -- Elements
	    ...
	    Tab3:AddLabel("Toggle Key : RightControl")
	    
	    Tab3:AddButton("Third Person", function()
	        local lplr = game.Players.LocalPlayer.Name
	        _G.LLPPLT = lplr
			workspace.Game.Players[_G.LLPPLT].WeaponController.ThirdPerson:Fire()
		end)
	    
	    local Slider1 = Tab:AddSlider("WalkSpeed", function(x)
			_G.NewWalkSpeed = x
		...
		Tab:AddButton("Change Walk Speed", function()
			loadstring(game:HttpGet("https://raw.githubusercontent.com/Yaki-0/Roblox/main/scripts/Evade%20WalkSpeed.lua"))()
		end)
		
	    local Slider1 = Tab:AddSlider("JumpPower", function(x)
			_G.NewWalkSpeed = x
		end, { -- (options are optional)
			["min"] = 0, -- Default: 0
			["max"] = 500, -- Default: 100
		...
		Tab:AddButton("Change Jump Power", function()
			loadstring(game:HttpGet("https://raw.githubusercontent.com/Yaki-0/Roblox/main/scripts/Evade%20WalkSpeed.lua"))()
		end)
	    
        local highlite = Instance.new("Highlight")
		highlite.Name = "ESP"
		highlite.OutlineColor = Color3.fromRGB(143, 67, 255)
		highlite.FillTransparency = 1
		highlite.Enabled = false
        
		local Switch = Tab2:AddSwitch("ESP", function(bool)
			highlite.Enabled = bool
		end)
		
		while wait() do
        for i,v in pairs(game:GetService("Workspace"):GetDescendants()) do
			if v.Name == "Humanoid" and v.Parent.ClassName == "Model" then
				if v.Parent.Name == game.Players.LocalPlayer.Name then
				else
				    if v:FindFirstChild("ESP") then
				    else
					   highlite.Parent = v.Parent
					end
				end
			end
		end
